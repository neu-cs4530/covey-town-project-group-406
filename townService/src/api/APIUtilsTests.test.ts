...
describe('createArtwork Tests', () => {
  ...
  it(`should create an artwork object for an invalid artwork's ID`, async () => {
    const artwork1: Artwork = await utils.createArtwork(invalidArtID);
    expect(artwork1.description).toEqual(
  ...
  it('should throw an error if no artwork exists with that ID', async () => {
    await expect(utils.createArtwork(nonExistantID)).rejects.toThrowError(`objectID doesn't exist`);
  });
});
...
describe('nextArtworks tests', () => {
  ...
  beforeAll(async () => {
    artworkList = await utils.nextArtworks(0, 10);
  });
